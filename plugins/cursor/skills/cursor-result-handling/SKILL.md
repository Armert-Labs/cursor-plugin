...
- If Cursor made edits (a write-capable rescue), say so explicitly. Cursor does not always enumerate touched files; if it does not, you may run `git status` to list them, but note that as a derived list rather than something Cursor reported.
