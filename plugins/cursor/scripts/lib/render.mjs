...
function formatCursorResumeCommand(job) {
  ...
  return `cursor-agent --resume ${job.chatId}`;
}
...
function formatUsageLine(usage) {
  ...
  if (Number.isFinite(usage.cacheReadTokens) && usage.cacheReadTokens > 0) {
...
function appendActiveJobsTable(lines, jobs) {
  ...
  for (const job of jobs) {
    ...
    lines.push(
      `| ${escapeMarkdownCell(job.id)} | ${escapeMarkdownCell(job.kindLabel)} | ${escapeMarkdownCell(job.status)} | ${escapeMarkdownCell(job.phase ?? "")} | ${escapeMarkdownCell(job.elapsed ?? "")} | ${escapeMarkdownCell(job.chatId ?? "")} | ${escapeMarkdownCell(job.summary ?? "")} | ${actions.map((action) => `\`${action}\``).join("<br>")} |`
    );
  }
}
...
function pushJobDetails(lines, job, options = {}) {
  ...
  if (job.chatId) {
    lines.push(`  Cursor chat ID: ${job.chatId}`);
  }
...
export function renderSetupReport(report) {
  const lines = [
    ...
    `- cursor-agent: ${report.cursorAgent.detail}`,
...
export function renderStoredJobResult(job, storedJob) {
  const chatId = storedJob?.chatId ?? job.chatId ?? null;
  const resumeCommand = chatId ? `cursor-agent --resume ${chatId}` : null;
  const usageLine = formatUsageLine(storedJob?.result?.usage);

  function withFooter(output) {
    let result = output.endsWith("\n") ? output : `${output}\n`;
    if (chatId) {
  ...
  if (chatId) {
    lines.push(`Cursor chat ID: ${chatId}`);
    lines.push(`Resume in Cursor: ${resumeCommand}`);
  }
