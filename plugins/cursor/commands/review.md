...
Run a Cursor code review of the local git state. Cursor reviews read-only (`--mode ask`); it does not edit anything.
...
- A specific model can be chosen with `--model` (e.g. `--model review` for a strong reviewer, `--model opus` for a Claude cross-check).
