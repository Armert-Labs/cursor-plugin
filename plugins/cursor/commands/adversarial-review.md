...
Run an adversarial Cursor review of the local git state. Cursor reviews read-only (`--mode ask`); it does not edit anything.
...
- `/cursor:adversarial-review` uses the same review target selection as `/cursor:review` and supports working-tree review, branch review, and `--base <ref>`.
- Unlike `/cursor:review`, it can take extra focus text after the flags (e.g. "focus on the retry logic").
- A specific model can be chosen with `--model`.
